...
module.exports = {
  sendFile (bucketName, key, fsPath, callback) {
    ...
    const putEventEmiter = s3Client.putFile(fsPath, key, (err, res) => {
      if (err != null) {
      ...
      return LocalFileWriter.deleteFile(fsPath, err => {
        logger.log({ res, bucketName, key, fsPath }, 'file uploaded to s3')
        return callback(err)
      })
    })
    return putEventEmiter.on('error', err => {
  ...
  copyFile (bucketName, sourceKey, destKey, callback) {
    ...
    return s3Client.copyFile(sourceKey, destKey, err => {
      if (err != null) {
  ...
  deleteDirectory (bucketName, key, _callback) {
    ...
    return s3Client.list({ prefix: key }, (err, { Contents }) => {
      if (err != null) {
        logger.err({ err, bucketName, key }, 'something went wrong listing prefix in aws')
        return callback(err)
      }
      const keys = _.map(Contents, ({ Key }) => Key)
