...
const checkCanStoreFiles = callback => {
  ...
  return keyBuilder.userFileKey(req, res, () => {
    fileController.getFile(req, myWritableStreamBuffer)
    return myWritableStreamBuffer.on('close', () => {
...
const checkFileConvert = callback => {
  const imgPath = path.join(settings.path.uploadFolder, '/tiny.pdf')
  return async.waterfall(
    [cb => fs.copy('./tiny.pdf', imgPath, cb), cb => fileConverter.thumbnail(imgPath, cb), (resultPath, cb) => fs.unlink(resultPath, cb), cb => fs.unlink(imgPath, cb)],
    callback
  )
}
...
module.exports = {
  check (req, res) {
    ...
    return async.parallel([checkFileConvert, checkCanStoreFiles], err => {
      if (err != null) {
